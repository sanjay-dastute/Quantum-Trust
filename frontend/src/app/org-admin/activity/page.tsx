...
export default function ActivityTrackerPage() {
  ...
  const fetchActivity = async () => {
    try {
      ...
      if (!orgId) return;
      const res = await fetch(`http://localhost:3000/api/org/${orgId}/activity`, {
  ...
  return (
    <Box sx={{ p: 4 }}>
      <Typography variant="h4" sx={{ mb: 4 }}>Team Activity Timeline</Typography>
              ...
              <Box key={day._id}>
                ...
                {day.actions.map((act: any, i: number) => (
                  <ListItem key={i} disablePadding sx={{ py: 1 }}>
                    <ListItemText 
                      primary={`${act.username} performed ${act.action}`} 
