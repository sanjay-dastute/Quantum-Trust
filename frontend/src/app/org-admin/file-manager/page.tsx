...
export default function OrgFileManagerPage() {
  ...
  const fetchPreview = async (dataId: string) => {
    const token = localStorage.getItem('accessToken');
    const res = await fetch(`http://localhost:3000/api/org/view-data?data_id=${dataId}`, {
      method: 'GET',
      headers: { Authorization: `Bearer ${token}` }
    });
  ...
  const executeSingleEncryption = async (dataId: string) => {
    const token = localStorage.getItem('accessToken');
    const res = await fetch('http://localhost:3000/api/org/encrypt', {
      method: 'POST',
      headers: {  
